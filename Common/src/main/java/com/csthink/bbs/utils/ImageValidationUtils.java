...
public class ImageValidationUtils {
    ...
    public static int VERIFY_SIZE = 4;

    public static int IMAGE_WIDTH = 100;

    public static int IMAGE_HEIGHT = 36;
    ...
    public static String generateVerifyCode() {
        ...
        for (int i = 0; i < VERIFY_SIZE; i++) {
            verifyCode.append(VERIFY_CODES.charAt(rand.nextInt(codesLength - 1)));
        }
    ...
    public static void outputImage(OutputStream os, String code) throws IOException {
        ...
        for (int i = 0; i < 20; i++) {
            int x = random.nextInt(IMAGE_WIDTH - 1);
            int y = random.nextInt(IMAGE_HEIGHT - 1);
        ...
        shear(g2, IMAGE_WIDTH, IMAGE_HEIGHT, c); // 使图片扭曲

        g2.setColor(getRandColor(100, 160));
        int fontSize = IMAGE_HEIGHT - 4;
        Font font = new Font("Algerian", Font.ITALIC, fontSize);
        g2.setFont(font);
        char[] chars = code.toCharArray();
        for (int i = 0; i < verifySize; i++) {
            AffineTransform affine = new AffineTransform();
            affine.setToRotation(Math.PI / 4 * rand.nextDouble() * (rand.nextBoolean() ? 1 : -1), (IMAGE_WIDTH / verifySize) * i + fontSize / 2, IMAGE_HEIGHT / 2);
            g2.setTransform(affine);
            g2.drawChars(chars, i, 1, ((IMAGE_WIDTH - 10) / verifySize) * i + 5, IMAGE_HEIGHT / 2 + fontSize / 2 - 10);
        }
